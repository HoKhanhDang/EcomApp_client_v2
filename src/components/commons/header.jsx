...
export default function Header() {
    ...
    const handleSetKeyword = (e) => {
        const str = e.target.value.split(" ").filter((item) => item !== "");
        setKeyword(e.target.value);
        if (str.length > 0) {
            const combinedKeywords = str.join("+");
            navigate(`/${path.SEARCH}?key=${combinedKeywords}`);
        } else {
            setKeyword("");
        }
    };
    ...
    return (
        <div
            id="home"
            class="w-full h-[140px] flex flex-row justify-between md:justify-evenly items-center"
        >
            <Link
                to="/"
                class="w-1/2 md:w-1/6 flex justify-center items-center p-[5px]"
            >
                <img
            ...
            <div class="w-4/6 hidden md:flex flex-row justify-center items-center gap-5 ">
                <div className="w-[60%] h-[60px] border rounded-[100px] px-[35px] flex justify-center items-center relative">
                    ...
                    <div className="h-full w-auto absolute right-5 opacity-50 flex justify-center items-center ">
                        <GoSearch className=" text-[40px] opacity-90 hover:text-main-100" />
                    </div>
                </div>
            </div>
