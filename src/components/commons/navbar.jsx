...
//api
import { apiGetCategory, apiGetBrand } from "../../apis/apis";
export default function Navbar() {
    ...
    const fetchData = async () => {
        const rs = await Promise.all([apiGetCategory(), apiGetBrand()]);
        setBrands(rs[1].data);
            ...
            <div class="w-[400px] md:w-auto h-[30px] hidden md:flex flex-row justify-start items-center font-main font-medium">
                {contantsNav.map((item, index) => {
                    return (
                        ...
                        >
                            <Dropdown items={item} brands={brands} categories={categories} />
                        </NavLink>
                    );
                })}
            </div>
